...
public class ProcMushroom : ProcBase
{
	...
	//data to define the B�zier handles for the cap curve:
	...
	public override Mesh BuildMesh()
	{
		...
		//B�zier handles to define the cap curve:
		...
		test.transform.position = currentOffset + gameObject.transform.position;
		test.transform.rotation = currentRotation;
		...
		// //adjust the B�zier handles for our inner curve:
	...
	/// <param name="capRim">Position at the rim of the cap (in XY cross-section), or the start point of the B�zier curve.</param>
	/// <param name="capPeak">Position at the peak of the cap (in XY cross-section), or the end point of the B�zier curve.</param>
	/// <param name="controlRim">The first of the inner control points of the B�zier curve (the rim position plus the rim handle).</param>
	/// <param name="controlPeak">The second of the inner control points of the B�zier curve (the peak position plus the peak handle).</param>
	private void BuildCap(MeshBuilder meshBuilder, Vector3 offset, Quaternion rotation, Vector3 capRim, Vector3 capPeak, Vector3 controlRim, Vector3 controlPeak)
	{
		...
		for (int i = 0; i <= capHeightSegmentCount; i++)
		{
			...
			//interpolated position along the B�zier curve:
			Vector3 bezier = Bezier(capRim, controlRim, controlPeak, capPeak, heightNormalised);

			//calculate a height offset and radius based on the B�zier curve position:
			Vector3 centrePos = new Vector3(0.0f, bezier.y, 0.0f);
			float radius = bezier.x;

			//interpolated tangent along the B�zier curve:
	...
	/// <summary>
	/// Gets a position along a B�zier curve.
	/// 
	/// For more information:
	/// http://pomax.github.io/bezierinfo/
	/// </summary>
	/// <param name="start">The first of the four control points for the B�zier curve.</param>
	/// <param name="controlMid1">The second of the four control points for the B�zier curve.</param>
	/// <param name="controlMid2">The third of the four control points for the B�zier curve.</param>
	/// <param name="end">The last of the four control points for the B�zier curve.</param>
	/// <param name="t">The interpolation value, should be between 0.0 and 1.0</param>
	/// <returns>The interpolated position along the B�zier curve.</returns>
	...
	/// <summary>
	/// Gets a tangent along a B�zier curve.
	/// 
	/// For more information:
	/// http://pomax.github.io/bezierinfo/
	/// </summary>
	...
	/// <param name="end">The last of the four control points for the B�zier curve.</param>
	/// <param name="t">The interpolation value, should be between 0.0 and 1.0</param>
	/// <returns>The interpolated tangent along the B�zier curve.</returns>
